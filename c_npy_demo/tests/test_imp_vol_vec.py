...
from .fixtures import (edo_full_data, edo_ntm_data, hh_full_data, hh_ntm_data,
                       rf_stop_defaults)
...
def test_ntm_imp_vol_vec(edo_ntm_data, hh_ntm_data, rf_stop_defaults, method,
    ...
    voas_edo = ndarray2vol_obj_args_array(edo_ntm_data, mult = mult)
...
@pytest.mark.parametrize("method", ["halley", "newton"])
@pytest.mark.parametrize("guess", [0.5, 0.7, 1]) # default guess is 0.5
@pytest.mark.parametrize("mult", [1, 10]) # highest tested was 1000
@pytest.mark.parametrize("py_debug", [False])
def test_full_imp_vol_vec(edo_full_data, hh_full_data, rf_stop_defaults, method,
                          guess, n_threads, mult, py_debug):
    ...
    :param edo_full_data: ``pytest`` fixture. See ``fixtures.py``.
    :type edo_full_data: :class:`numpy.ndarray`.
    :param hh_full_data: ``pytest`` fixture. See ``fixtures.py``.
    :type hh_full_data: :class:`numpy.ndarray`.
    :param rf_stop_defaults: ``pytest`` fixture. See ``fixtures.py``.
    :type rf_stop_defaults: tuple
    :param method: Method to use to solve, either ``"halley"`` or ``"newton"``.
    :type method: str
    ...
    """
    # convert ndarray options data to ctypes array of vol_obj_args
    voas_edo = ndarray2vol_obj_args_array(edo_full_data, mult = mult)
